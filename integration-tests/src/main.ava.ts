...
test.beforeEach(async (t) => {
  ...
  t.context.accounts = { root, contract, beneficiary, alice, bob, john };
});
...
test("Alice sends donations to the beneficiary", async (t) => {
...
test("Bob sends donations to the beneficiary", async (t) => {
  const { contract, bob, beneficiary } = t.context.accounts;

  const balance = await beneficiary.balance();
  const available = parseFloat(balance.available.toHuman());

  await bob.call(contract, "donate", {}, { attachedDeposit: NEAR.parse("1 N").toString() });
...
test("John send and consult the donation", async (t) => {
  const { contract, john } = t.context.accounts;

  await john.call(contract, "donate", {}, { attachedDeposit: NEAR.parse("2 N").toString() });

  const donation: Donation = await contract.view("get_donation_for_account", { account_id: john.accountId });
  
  t.is(donation.account_id, john.accountId);
